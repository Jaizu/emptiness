            ...
            case EVO_LEVEL_DARK_TYPE_MON_IN_PARTY:
                if (gEvolutionTable[species][i].param <= level)
                {
                    for (j = 0; j < PARTY_SIZE; j++)
                    {
                        ...
                        if (gBaseStats[species].type1 == TYPE_DARK || gBaseStats[species].type2 == TYPE_DARK)
                        {
                            targetSpecies = gEvolutionTable[species][i].targetSpecies;
                            break;
                        }
...
{
    return (species != SPECIES_CASTFORM && species != SPECIES_DEOXYS && species != SPECIES_SPINDA && species != SPECIES_UNOWN && species != SPECIES_CHERRIM);
}
